...
#include <vector> // Incluir la biblioteca para utilizar std::vector
// Variable global para almacenar los valores JSON
std::vector<String> valoresJSON;
...
void GETJSON() {
  ...
  // Dirección URL a la que deseas enviar la solicitud GET
  String url = "http://192.168.31.120:8500/arduino/";

  Serial.print("Enviando solicitud GET a: ");
  Serial.println(url);

  http.begin(url);  // Especificar URL
  int httpCode = http.GET();  // Realizar la solicitud GET

  if (httpCode > 0) { // Verificar si la solicitud fue exitosa
    Serial.printf("[HTTP] Código de estado: %d\n", httpCode);
    String payload = http.getString();   // Obtener respuesta del servidor
    Serial.println("Respuesta del servidor:");
    Serial.println(payload);             // Mostrar respuesta en el puerto serie
    ...
    if (error) {
      ...
      // Extraer los valores del JSON y almacenarlos en el vector global
      valoresJSON.clear(); // Limpiar el vector antes de agregar nuevos valores
      for (JsonPair kvp : jsonDocument.as<JsonObject>()) {
        valoresJSON.push_back(kvp.value().as<String>());
      }
    }
  } else {
